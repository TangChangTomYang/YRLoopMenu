...
@implementation YRLoopMunuLayout {
    NSInteger _index;
}
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    CGFloat visibleRectX = self.collectionView.contentOffset.x;
    CGFloat visibleRectY = self.collectionView.contentOffset.y;
    CGFloat visibleRectW = self.collectionView.bounds.size.width;
    CGFloat visibleRectH = self.collectionView.bounds.size.height;
    CGRect visibleRect = CGRectMake(visibleRectX, visibleRectY, visibleRectW, visibleRectH);
    ...
    for (UICollectionViewLayoutAttributes *attributes in visibleItemArray) {
        ...
        if (self.threeDimensionalScale>0) {
            CGFloat sx = 1 + scale * (self.threeDimensionalScale-1);
            CGFloat sy =  1 + scale * (self.threeDimensionalScale-1);
            attributes.transform3D = CATransform3DMakeScale(sx,sy,1);
        }
    ...
    if (indexPathNow.row == 0)  {
        ...
        if (newBounds.origin.x < SC_ScreenSize().width / 2)  {
            if (_index != indexPathNow.row)  {
                ...
                if ([self.delegate respondsToSelector:@selector(collectioViewDidScrollToIndex:)])   {
                    [self.delegate collectioViewDidScrollToIndex:_index];
                }
